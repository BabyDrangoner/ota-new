...
retry:
    ...
    if(n == -1 && errno == EAGAIN) {
        SYLAR_LOG_DEBUG(g_logger) << "do_io<" << hook_fun_name << ">" << "use IOManager";
        SYLAR_LOG_DEBUG(g_logger) << "ctx->getUserNonblock() = " << ctx->getUserNonblock();
        ...
        if(rt) {
            ...
            SYLAR_LOG_DEBUG(g_logger) << "do_io<" << hook_fun_name << ">" << "do retry";

            goto retry;
        }
    }

    SYLAR_LOG_DEBUG(g_logger) << "do_io<" << hook_fun_name << ">" << "return"
                              << "n = " << n
...
unsigned int sleep(unsigned int seconds) {
    ...
    iom->addTimer(seconds * 1000, std::bind((void(sherry::Scheduler::*)
            (sherry::Fiber::ptr, int thread))&sherry::IOManager::schedule
            ,iom, fiber, -1));
    sherry::Fiber::YieldToHold();
    return 0;
}
...
int usleep(useconds_t usec) {
    ...
    sherry::Fiber::ptr fiber = sherry::Fiber::GetThis();
    sherry::IOManager* iom = sherry::IOManager::GetThis();
    iom->addTimer(usec / 1000, std::bind((void(sherry::Scheduler::*)
            (sherry::Fiber::ptr, int thread))&sherry::IOManager::schedule
            ,iom, fiber, -1));
    sherry::Fiber::YieldToHold();
    return 0;
...
int nanosleep(const struct timespec *req, struct timespec *rem) {
    ...
    sherry::Fiber::ptr fiber = sherry::Fiber::GetThis();
    sherry::IOManager* iom = sherry::IOManager::GetThis();
    iom->addTimer(timeout_ms, std::bind((void(sherry::Scheduler::*)
            (sherry::Fiber::ptr, int thread))&sherry::IOManager::schedule
            ,iom, fiber, -1));
    sherry::Fiber::YieldToHold();
    return 0;
...
ssize_t sendfile(int s, int f, off_t* offset, size_t count) {
    return do_io(s, sendfile_f, "sendfile", sherry::IOManager::WRITE, SO_SNDTIMEO, f, offset, count);
}
