...
#include "sherry/db/fox_thread.h"
#include "sherry/singleton.h"
...
public:
    ...
    FoxRedis(sherry::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
private:
    struct FCtx {
        std::string cmd;
        sherry::Scheduler* scheduler;
        sherry::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
public:
    ...
    FoxRedisCluster(sherry::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
