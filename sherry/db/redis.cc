...
#include "sherry/util/hash_util.h"

namespace sherry {

static sherry::Logger::ptr g_logger = SYLAR_LOG_NAME("system");
static sherry::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_redis =
    sherry::Config::Lookup("redis.config", std::map<std::string, std::map<std::string, std::string> >(), "redis config");
...
Redis::Redis(const std::map<std::string, std::string>& conf) {
    ...
    m_port = sherry::TypeUtil::Atoi(tmp.substr(pos + 1));
    m_passwd = get_value(conf, "passwd");
    m_logEnable = sherry::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));

    tmp = get_value(conf, "timeout_com");
    if(tmp.empty()) {
        tmp = get_value(conf, "timeout");
    }
    uint64_t v = sherry::TypeUtil::Atoi(tmp);
...
FoxRedis::FoxRedis(sherry::FoxThread* thr, const std::map<std::string, std::string>& conf)
    :m_thread(thr)
...
bool FoxRedis::init() {
    if(m_thread == sherry::FoxThread::GetThis()) {
        return pinit();
    } else {
...
bool FoxRedis::pinit() {
    ...
    m_context.reset(ctx, sherry::nop<redisAsyncContext>);
...
ReplyPtr FoxRedis::cmd(const char* fmt, va_list ap) {
    ...
    //    free(buf);
    //}
    //ctx->scheduler = sherry::Scheduler::GetThis();
    //ctx->fiber = sherry::Fiber::GetThis();
    //ctx->thread = m_thread;

    FCtx fctx;
    fctx.cmd.append(buf, len);
    free(buf);
    fctx.scheduler = sherry::Scheduler::GetThis();
    fctx.fiber = sherry::Fiber::GetThis();
...
FoxRedis::Ctx::Ctx(FoxRedis* r)
    ...
    //,scheduler(nullptr)
    ,thread(nullptr) {
    sherry::Atomic::addFetch(rds->m_ctxCount, 1);
}

FoxRedis::Ctx::~Ctx() {
    //cancelEvent();
    SYLAR_ASSERT(thread == sherry::FoxThread::GetThis());
    //SYLAR_ASSERT(destory == 0);
    sherry::Atomic::subFetch(rds->m_ctxCount, 1);
    //++destory;
    //cancelEvent();
    if(ev) {
        evtimer_del(ev);
...
void FoxRedis::Ctx::cancelEvent() {
    //if(ev) {
    //    if(thread == sherry::FoxThread::GetThis()) {
    //        evtimer_del(ev);
    //        event_free(ev);
    //    } else {
    //        auto e = ev;
...
FoxRedisCluster::FoxRedisCluster(sherry::FoxThread* thr, const std::map<std::string, std::string>& conf)
    :m_thread(thr)
    ,m_status(UNCONNECTED)
    ,m_event(nullptr) {
    m_ctxCount = 0;
...
bool FoxRedisCluster::pinit() {
    ...
    m_status = CONNECTED;
    //m_context.reset(ctx, redisAsyncFree);
    m_context.reset(ctx, sherry::nop<redisClusterAsyncContext>);
...
void FoxRedisCluster::Ctx::cancelEvent() {
    //SYLAR_LOG_INFO(g_logger) << "cancelEvent " << sherry::FoxThread::GetThis()
    //           << " - " << thread
    //           << " - " << sherry::IOManager::GetThis()
    //           << " - " << cancel_count;
    //if(thread != sherry::FoxThread::GetThis()) {
    //    SYLAR_LOG_INFO(g_logger) << "cancelEvent " << sherry::FoxThread::GetThis()
    //               << " - " << thread
    ...
    //    //           << " this=" << sherry::FoxThread::GetThis();
    //    //SYLAR_ASSERT(thread == sherry::FoxThread::GetThis());
    //}
    //SYLAR_ASSERT(!sherry::IOManager::GetThis());
    ////if(sherry::Atomic::addFetch(cancel_count) > 1) {
    ////    return;
    ////}
    ////SYLAR_ASSERT(!sherry::Fiber::GetThis());
    ////sherry::RWMutex::WriteLock lock(mutex);
    //if(++cancel_count > 1) {
    //    return;
    //}
    //if(ev) {
...
IRedis::ptr RedisManager::get(const std::string& name) {
    sherry::RWMutex::WriteLock lock(m_mutex);
    ...
    if(r->getType() == IRedis::FOX_REDIS
        ...
        return std::shared_ptr<IRedis>(r, sherry::nop<IRedis>);
    }
    lock.unlock();
...
void RedisManager::init() {
    ...
    for(auto& i : m_config) {
        ...
        auto pool = sherry::TypeUtil::Atoi(get_value(i.second, "pool"));
        auto passwd = get_value(i.second, "passwd");
        total += pool;
        for(int n = 0; n < pool; ++n) {
            if(type == "redis") {
                sherry::Redis* rds(new sherry::Redis(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                sherry::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                sherry::Atomic::addFetch(done, 1);
            } else if(type == "redis_cluster") {
                sherry::RedisCluster* rds(new sherry::RedisCluster(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                sherry::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ...
                auto conf = i.second;
                auto name = i.first;
                sherry::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done](){
                    sherry::FoxRedis* rds(new sherry::FoxRedis(sherry::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
                ...
                auto name = i.first;
                sherry::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done](){
                    sherry::FoxRedisCluster* rds(new sherry::FoxRedisCluster(sherry::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
