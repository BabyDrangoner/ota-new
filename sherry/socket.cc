...
Socket::ptr Socket::CreateTCP(sherry::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(sherry::Address::ptr address) {
...
int Socket::sendFile(int fileFd, off_t* offset, size_t length){
    if(isConnected()){
        return ::sendfile(m_sock, fileFd, offset, length);
    }
    return -1;
}
...
int Socket::recv(void* buffer, size_t length, int flags) {
    ...
    SYLAR_LOG_DEBUG(g_logger) << "socket disconnected";
    return -1;
}
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, sherry::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, sherry::IOManager::WRITE);
}
...
SSLSocket::ptr SSLSocket::CreateTCP(sherry::Address::ptr address) {
