...
class EchoServer : public sherry::TcpServer {
public:
    EchoServer(int type);
    void handleClient(sherry::Socket::ptr client);
...
void EchoServer::handleClient(sherry::Socket::ptr client) {
    SYLAR_LOG_INFO(g_logger) << "handleClient " << *client;   
    sherry::ByteArray::ptr ba(new sherry::ByteArray);
    while(true) {
        ba->clear();
